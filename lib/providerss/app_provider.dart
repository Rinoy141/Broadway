...
class MainProvider extends ChangeNotifier {
  ...
  //               Uri.parse(url)), // Include cookie if required
  //           'Accept': 'application/json',
  ...
  Future<void> applyPromoCode(BuildContext context, String code) async {
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        final message =
            response.data['msg'] ?? 'Promo code applied successfully.';
  ...
  Future<bool> placeOrder(
      BuildContext context, String selectedPaymentMethod) async {
    try {
      ...
      if (cookieString.isEmpty) {
        throw Exception(
            'Session cookie is empty. User might not be logged in.');
      }
  ...
  Future<void> fetchRestaurantDetails(int restaurantId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('Restaurant Details JSON: $restaurantDetailsJson');
  ...
  Future<bool> addRestaurantReview({
    ...
    try {
      ...
      if (rating < 1 || rating > 5) {
        _showErrorDialog(
            context, 'Please provide a valid rating between 1 and 5.');
        return false;
      }
      ...
      if (response.statusCode == 200 &&
        ...
        _showSuccessDialogs(
            context, 'Thank you! Your review has been submitted.');
        return true;
      } else {
  ...
  Future<void> fetchRestaurantMenu(int restaurantId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Failed to fetch restaurant menu, Status Code: ${response.statusCode}');
      }
    } catch (e) {
