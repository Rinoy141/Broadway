...
class CustomizationPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final itemId = popularItem?.id ?? item?.id ?? 0;
    final price = popularItem?.price ??
        (item?.prices['default'] is num
            ? item!.prices['default'].toDouble()
            : 0.0);
                              ...
                              : () async {
                                  ...
                                  if (result && context.mounted) {
                                    ...
                                    if (provider.error ==
                                      ...
                                      _showPortionSelectionBottomSheet(
                                          context, itemId, provider);
                                    }
                                  }
                                },
                          style: ElevatedButton.styleFrom(
  ...
  void _showPortionSelectionBottomSheet(
      BuildContext context, int itemId, MainProvider provider) {
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              ListView.builder(
                shrinkWrap: true,
                itemCount: provider.portionOptions.length,
                itemBuilder: (context, index) {
                  final portionOption = provider.portionOptions[index];
                  ...
                  return ListTile(
                    ...
                    trailing: Text(
                      '\$${portionOption.price.toStringAsFixed(2)}',
                      style: const TextStyle(
                        fontSize: 16,
                        fontWeight: FontWeight.bold,
                      ),
                    ...
                    onTap: () async {
                      provider.selectPortionSize(portionOption.name);
                      final success = await provider.addCustomizableItemToCart(
                        itemId,
                      );
